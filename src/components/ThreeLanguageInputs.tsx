...
export function ThreeLanguageInputs({
  ...
  const canProceed = () => {
    ...
    switch (currentStep) {
      case "en":
        return values[fieldNames.en]?.trim();
      case "ka":
        return values[fieldNames.ka]?.trim();
      case "ru":
        return values[fieldNames.ru]?.trim();
      default:
        return false;
    }
  };
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div>
        <label
          ...
          onClick={() => setCurrentStep("en")}
        >
          {labels.en}{" "}
          {currentStep === "en" && (
            <span className="text-blue-500">(current)</span>
          )}
          {watchedValues[fieldNames.en] && currentStep !== "en" && (
        ...
        {errors?.[fieldNames.en] && (
          <p className="text-red-500 text-sm mt-1">
            {String(errors[fieldNames.en]?.message || "")}
          </p>
        )}
      </div>
      ...
      {(currentStep === "ka" ||
        ...
        watchedValues[fieldNames.ka] ||
        watchedValues[fieldNames.en]) && (
        <div>
          <label
            className="block text-sm font-medium mb-2 cursor-pointer"
            onClick={() => watchedValues[fieldNames.en] && setCurrentStep("ka")}
          >
            {labels.ka}{" "}
            {currentStep === "ka" && (
              <span className="text-blue-500">(current)</span>
            )}
            {watchedValues[fieldNames.ka] && currentStep !== "ka" && (
              <span className="text-green-500 ml-2">✓</span>
            )}
          </label>
          <Input
          ...
          {errors?.[fieldNames.ka] && (
            <p className="text-red-500 text-sm mt-1">
              {String(errors[fieldNames.ka]?.message || "")}
            </p>
          )}
        </div>
      )}
      ...
      {(currentStep === "ru" ||
        currentStep === "complete" ||
        watchedValues[fieldNames.ru] ||
        (watchedValues[fieldNames.en] && watchedValues[fieldNames.ka])) && (
        <div>
          <label
            className="block text-sm font-medium mb-2 cursor-pointer"
            onClick={() => watchedValues[fieldNames.ka] && setCurrentStep("ru")}
          >
            {labels.ru}{" "}
            {currentStep === "ru" && (
              <span className="text-blue-500">(current)</span>
            )}
            {watchedValues[fieldNames.ru] && currentStep !== "ru" && (
              <span className="text-green-500 ml-2">✓</span>
            )}
          </label>
          <Input
          ...
          {errors?.[fieldNames.ru] && (
            <p className="text-red-500 text-sm mt-1">
              {String(errors[fieldNames.ru]?.message || "")}
            </p>
          )}
        </div>
      )}
