      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t("adminDashboard.topMasters.title")}
          </CardTitle>
        </CardHeader>
        <CardContent>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {master.completedJobs} jobs • ${master.totalEarnings}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-semibold">{master.totalPoints} pts</p>
                    {master.avgRating && (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t("adminDashboard.geographicOverview.title")}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <div>
              <h4 className="font-medium mb-2">
                {t("adminDashboard.geographicOverview.mastersByCity")}
              </h4>
              <div className="space-y-2">
                {data.geographicalData.mastersByCity.map((city) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <span className="font-semibold">{city.masterCount}</span>
            ...
            <div>
              <h4 className="font-medium mb-2">
                {t("adminDashboard.geographicOverview.serviceLocations")}
              </h4>
              {data.geographicalData.servicesByLocation.map((location) => (
                <div key={location.location} className="p-2 bg-blue-50 rounded">
                  <p className="font-medium text-sm">{location.location}</p>
                  <p className="text-xs text-gray-600">
                    {location.requestCount} requests • ${location.totalValue}{" "}
                    total
                  </p>
                </div>
              ))}
